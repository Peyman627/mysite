...
class TestsQuestionModel(TestCase):
...
class TestsQuestionIndexView(TestCase):
    ...
    def test_fututre_question(self):
        """
        Question with a pub_date in the future are
    ...
    def test_fututre_question_and_past_question(self):
        """
        Even if both past and future questions exist,
        only past questions are dispayed.
        """
        create_question(question_text="Past question.", days=-30)
        create_question(question_text="Future question.", days=30)
        response = self.client.get(reverse('polls:index'))
